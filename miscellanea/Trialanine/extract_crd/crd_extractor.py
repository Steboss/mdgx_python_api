...
def find_zmatrix(reader,stationary_index,natoms,template):

    last_idx = len(stationary_index)
    end_reader=len(reader)
    phi = 180  #startig values of phi
    ...
    for i in range(0,last_idx):
        ...
        gzmatwriter(extract,natoms,phi,template)
        phi +=10
...
def crdwriter(zmatrix,natoms,phi):
    ...
    outcrdname = "crd/structure_%d.crd"  %phi
...
def gzmatwriter(reader,natoms,phi,template):
    ...
    outputgzmat.write("\n")
    outputgzmat.write("Optimization of phi %d\n" %phi )
    outputgzmat.write("\n")
...
find_zmatrix(reader,stationary_index,number_atoms,template)
