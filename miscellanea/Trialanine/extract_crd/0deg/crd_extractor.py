...
def find_zmatrix(reader,stationary_index,natoms,template,opt_var):
    ...
    for i in range(0,last_idx):
        ...
        phi =  gzmatwriter(extract,natoms,opt_var,template)
...
def gzmatwriter(reader,natoms,opt,template):
    ...
    for key in list(dict_variable.keys()):
        if key==opt:
            ...
            print("key %s  = %s variable" %(key,opt))

            phi = (dict_variable[key]).split(".")[0]
            print(phi)
    ...
    outputgzmat.write("#P hf/6-31G* Opt=(Tight,CalcFC) Freq SCF(Conver=6) Test\n")
    outputgzmat.write("\n")
...
for f in reader[var_start:var_end]:
    if "Scan" in f:
        opt_var = f.split()[0]  #variable which is scanned
    else:
        continue
print("The variable is %s is it correct?" % opt_var)
####################################################
...
find_zmatrix(reader,stationary_index,number_atoms,template,opt_var)
